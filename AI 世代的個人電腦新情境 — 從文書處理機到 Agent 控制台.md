...
> Claude Code 的操作介面是終端機的命令列，但你也可以在 VS Code 裡直接呼叫它。
...
本機的 repo 只活在你的電腦裡。Remote 是在 GitHub 建立一份鏡像，讓你的工作有雲端備份。
...
> 對知識工作者來說，這個輕量的文字編輯器，
> 提供了一個 Agent 可以感知環境、執行動作、讀寫檔案的操作空間。
...
Claude Code  ──────────────────────────── 在你電腦裡工作的 AI Agent
  ...
  |     └─► Remote（GitHub）  ────────── 雲端備份與協作，push / pull
  |
