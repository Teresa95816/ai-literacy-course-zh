...
2022 年, 研究人員發現只要在 Prompt 中加入「請一步一步思考（Let's think step by step）」等引導語, 模型在數學計算、邏輯推理與常識判斷等任務上的表現便能明顯提升.
...
以「整理某公司的歷年融資紀錄並製作趨勢圖」為例, AI 會依序完成以下流程:
...
| 使用 | 無狀態 + 多輪對話 | Context Window: 模型每次能「看到」的內容上限 |
